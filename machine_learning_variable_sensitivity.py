...
Additional script for the wind_power_icing project, training regressors with
reduced number of
input variables.
This script
...
for lead_hour in lead_hours:
    ...
    # leave out 1 variable at a time. we leave out ploss and ploss_abs together since they carry nearly
    # the same information
    vars_to_leave_out = input_vars[:] +['no_icevars','only_obs', 'only_nwp']

    for leave_out_var in vars_to_leave_out:
        print('leaving out', leave_out_var)
        if leave_out_var == 'ploss_abs':
            continue
        elif leave_out_var == 'only_obs':
            trimmed_input_vars = obs_vars
        elif leave_out_var == 'only_nwp':
            trimmed_input_vars = fc_vars
        elif leave_out_var == 'no_icevars':
            trimmed_input_vars = input_vars[:]
            trimmed_input_vars.remove('qice')
            trimmed_input_vars.remove('qsnow')
            trimmed_input_vars.remove('qgraup')

        else:
            trimmed_input_vars = input_vars[:]
            trimmed_input_vars.remove(leave_out_var)
            if leave_out_var == 'ploss':
                # also remove ploss_abs in this case
                trimmed_input_vars.remove('ploss_abs')


        X_train = df_train[trimmed_input_vars]
        y_train = df_train[target_var]
        X_test = df_test[trimmed_input_vars]
        ...
        _df= pd.DataFrame({'truth':y_test, 'prediction':ensmean,
                           ...
                           'leave_out_var':leave_out_var})

        # persistence forecast
        _df['persistence'] = df_test['ploss']  ## note: this does not work when target is ploss_abs!!

        _df['month'] = _df.index.month

        _df['lead_hour'] = lead_hour

        res_all_lead_times.append(_df)
...
sns.barplot('leave_out_var','abse', data=final_df, ci=ci, color=sns.color_palette()[0])
