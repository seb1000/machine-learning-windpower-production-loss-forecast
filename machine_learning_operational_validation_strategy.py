...
additional script for the wind_power_icing project, using the operational validation scheme
...
name = 'oper_validation' # for plot names
...
for lead_hour in lead_hours:
    print('lead_hour', lead_hour)
    # arrange the data for this lead time
    df = combine_fc_and_station_data(lead_hour)

    # sort by date
    df = df.sort_index()
    # now loop over all dates, starting at some reasonable threshold (it does not make much sense to train only on 1 sample....
    min_n_days = 90
    all_dates = pd.DatetimeIndex(df.date_init).unique().sort_values()
    res = []
    for date in all_dates[min_n_days:]:
        print(date)
        latest_date_with_validation = date - pd.Timedelta(str(lead_hour)+'h')
        # depending on lead_time, we dont have a forecast started at this lead time
        # search the eralist forecast we actually have
        test_date = all_dates[np.where(all_dates<latest_date_with_validation)[0][-1]]
        avialable_dates = df['date_init'] < latest_date_with_validation
        df_train = df[df['date_init'] < test_date].copy()
        df_test = df[df['date_init'] == test_date].copy()
    ...
    res_all_lead_times.append(pd.concat(res))



final_df = pd.concat(res_all_lead_times)

final_df.to_pickle('final_df_'+name+'.csv')
final_df = pd.read_pickle('final_df_'+name+'.csv')
