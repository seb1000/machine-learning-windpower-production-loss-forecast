...
def train_test_split_chunks(df, chunk_length=10):
    ...
    # check whether we have smaples form all stations both in train and test

    assert(len(df_train['station'].unique()) == len(stations))
    assert(len(df_test['station'].unique()) == len(stations))
...
for lead_hour in lead_hours:
    ...
    for i in range(N_trains):
        ...
        _df= pd.DataFrame({'truth':y_test, 'prediction':y_test_predicted, 'realization':i,
...
res_all_lead_times = pd.concat(res_all_lead_times)

#%%------------------- now compute the mean of the realizations

# there is probably a better way to do this, but like this it works: create a list, each entry
# is everything for one realization, and then compute the mean over this list
final_df_predictions = 1/N_trains * sum([res_all_lead_times.query('realization=='+str(i))['prediction'] for i in range(N_trains)])
assert(len(final_df_predictions)*10 == len(res_all_lead_times))
final_df = pd.concat([res_all_lead_times.query('realization==0').drop(columns='prediction'),final_df_predictions],axis=1)
...
# plot observed production loss
df_sub = final_df[final_df['lead_hour']==0].copy()
df_sub['datetime'] = df_sub.index
df_sub['ploss'] = df_sub['truth']
season1 = df_sub[df_sub['datetime'] < '2014-06-01']
season2 = df_sub[df_sub['datetime'] > '2014-06-01']

plt.figure(figsize=(30,8))
plt.subplot(211)
sns.lineplot('datetime', 'ploss', hue='station', estimator=None,data=season1)
plt.title('season 2013-2014')
plt.xlabel('')
plt.ylabel('observed relative production loss [%]')
sns.despine()
plt.subplot(212)
sns.lineplot('datetime', 'ploss', hue='station', estimator=None,data=season2)
plt.title('season 2014-2015')
plt.xlabel('')
plt.ylabel('observed relative production loss [%]')
sns.despine()
plt.savefig('plots/obs_ploss_both_seasons.svg')
